        ...
        if product_type_default in csv_list_map and csv_list_map[product_type_default]:
            ...
            try:
                ...
                if df_default is not None and all(col in df_default.columns for col in ['model', 'code', 'is_default']):
                    ...
                    df_cleaned_default = df_default[required_cols].dropna(subset=['model', 'code']) # is_default 可以为空? 假设不为空
                    df_cleaned_default = df_cleaned_default.astype(str) # 全部转为字符串
                    ...
                    model_to_default_code = default_rows.drop_duplicates(subset=['model']).set_index('model')['code'].to_dict()
                    ...
                    for model_name_default in df_cleaned_default['model'].unique():
                        if model_name_default not in model_details_map: # 避免被后续产品类型覆盖
                            ...
                            default_code_for_model = model_to_default_code.get(model_name_default) # 获取默认 code

                            details = {
                                "default_code": default_code_for_model, # 存储默认 code (可能是 None)
                    ...
                    missing_cols = {'model', 'code', 'is_default'} - set(df_default.columns if df_default is not None else [])
                    logger.warning(f"CSV文件 '{csv_path_default}' 缺少必需列: {missing_cols}，无法为 {product_type_default} 生成模型详情。")
