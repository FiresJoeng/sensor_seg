...
1. **数据结构分层**  
   ...
   -**合并验证规则**: 
   - 严格验证合并条件：两个位号若在共用参数组中存在任意字段差异，必须拆分为独立条目
   - 示例：
     正确 ： 位号A/B共享28个参数，仅"介质"不同 → 创建两个设备列表条目
     错误 ： 将A/B合并到同一共用参数组

2.**字段名称和对应值逐字复制，要求严格统一**  
   - 参数名称必须与表格列标题 **完全一致**（包括中英文、空格、标点）。  
   - 参数值必须与表格单元格内容 **完全一致**（包括数字格式、符号、单位）。
   - 不同的参数和参数值要区分清楚，例如在同一行里会有多个参数和参数值，在最后输出的时候要把不同的参数和参数值分别对应上后区分开来输出。不要把它们误认为是一个参数和参数值。  
   - 示例：  
...
{
  "共用参数设备列表": [{
    "共用参数位号": ["353-TE-35301","353-TE-35302"],
    "共用参数": {
